        ...
        .then(res => {
          if (res.data.statusChanges) {
            const statuses = res.data.statusChanges.map(status => ({
              Hours: (
                ((status.endedAt || Date.now()) - status.startedAt) /
                1000 /
                60 /
                60
              ).toFixed(2),
  ...
  if (product) {
    return (
      <div className={props.className}>
              ...
              <div>
                ...
                <div className={classes.chips}>
                  {product.technologies.map((technology, i) => (
                    <Chip key={i} label={technology} color="secondary" />
                  ))}
                </div>
              </div>
            ) : null}
              ...
              <div>
                ...
                <div className={classes.chips}>
                  {product.components.map((component, i) => (
                    <Chip key={i} label={component} color="secondary" />
                  ))}
                </div>
              </div>
            ) : null}
              ...
              <div>
                ...
                <div className={classes.chips}>
                  {product.environmentRequirements.map((requirement, i) => (
                    <Chip key={i} label={requirement} color="secondary" />
                  ))}
                </div>
              </div>
            ) : null}
            ...
            <div className={classes.bottomChipGroup}>
              ...
              {product.productOwner ? (
                <div>
                  ...
                  <div>
                    <Chip
                      onClick={copyText(product.productOwner)}
                      variant="outlined"
                      color="primary"
              ...
              {product.salesPerson ? (
                <div>
                  ...
                  <div>
                    <Chip
                      onClick={copyText(product.salesPerson)}
                      variant="outlined"
                      color="primary"
        ...
        <Dialog title="Graph" isOpen={isDialogOpen} setOpen={setIsDialogOpen}>
          {graphData && (
            <div style={{ width: "100%" }}>
              <BarChart
                width={550}
